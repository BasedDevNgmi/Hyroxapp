...
function getPhaseForWeek(week: number): { label: string; color: string } {
  if (week <= 12) return { label: 'P1', color: 'bg-blue-500' }
  if (week <= 20) return { label: 'P2', color: 'bg-amber-500' }
  if (week <= 28) return { label: 'P3', color: 'bg-red-500' }
  if (week <= 38) return { label: 'P4', color: 'bg-purple-500' }
  return { label: 'P5', color: 'bg-emerald-500' }
}
...
function isTestWeek(week: number) {
  return TEST_WEEKS.has(week)
}
...
export default function DashboardPage() {
  ...
  function renderCalendarDay(day: Date, inMonth: boolean = true) {
    ...
    const deload = weekNum > 0 && weekNum <= 42 && isDeloadWeek(weekNum)
    const testWeek = weekNum > 0 && weekNum <= 42 && isTestWeek(weekNum)
    ...
    return (
      ...
      >
        ...
        {testWeek && inMonth && wo && (
          <span className="text-[7px] text-amber-400 leading-none">T</span>
        )}
      </button>
    )
  }
          ...
          <div className="px-4 pb-4">
            ...
            <div className="flex items-center gap-3 mt-2 pt-2 border-t border-border">
              ...
              <div className="flex items-center gap-1">
                ...
                <span className="text-[9px] text-muted-foreground">P4</span>
              </div>
              <div className="flex items-center gap-1">
                <div className="w-1.5 h-1.5 rounded-full bg-emerald-500" />
                <span className="text-[9px] text-muted-foreground">P5</span>
              </div>
              <div className="flex items-center gap-1">
                <CheckCircle2 className="w-2.5 h-2.5 text-green-500" />
